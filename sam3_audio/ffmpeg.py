...
def cut(src: Path, frag: Fragment, dst: Path, *, reencode: bool = False) -> None:
    """Cut ``frag`` from ``src`` to ``dst``.

    If *reencode* is False, tries stream-copy first (fast) and re-encodes on
    failure. If *reencode* is True, always re-encodes (needed for format
    conversion).
    """
    slice_args = [
        *_BASE,
...
def concat_cuts(
    ...
    *out_ext* overrides the intermediate file extension (e.g. ``".wav"``).
    When it differs from the source, all cuts are re-encoded.
    """
    dst.parent.mkdir(parents=True, exist_ok=True)
    ext = out_ext or dst.suffix or src.suffix
    reencode = out_ext is not None and out_ext != src.suffix
    with tempfile.TemporaryDirectory() as td:
        td_path = Path(td)
        parts: list[Path] = []
        for i, frag in enumerate(frags):
            part = td_path / f"p{i:04d}{ext}"
            cut(src, frag, part, reencode=reencode)
            parts.append(part)
...
def split_cuts(
    ...
    *out_ext* overrides the output extension (e.g. ``".wav"``).
    """
    out_dir.mkdir(parents=True, exist_ok=True)
    ext = out_ext or src.suffix
    reencode = out_ext is not None and out_ext != src.suffix
    for i, frag in enumerate(frags):
        cut(src, frag, out_dir / f"{src.stem}_{i + 1:02d}{ext}", reencode=reencode)
